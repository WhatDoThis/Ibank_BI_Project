 ...
 * [Main Functions]
 * ===========
 * 1. batchGetJobHistoryDetail(batchJobId, runId) 호출 후 요약·파일별 결과 표시
 * 2. 파일별: 상세 토글, 원격 삭제, 적재 롤백(PK 있는 배치만, 성공 파일만)
 * 3. file_list: filename, timestamp, status, rows(또는 inserted/updated), error 또는 reason(실패/스킵 시)
 *
 * [Props]
 * =====
 * 1. batchJobId: number — 배치 Job ID
 * 2. runId: number — 실행 이력 run_id
 * 3. onBack: () => void — 목록으로 돌아가기
 * 4. onClose: () => void — 모달 닫기
 *
 * [Dependencies]
 * =========
