"""
Backend.etl_server2.router_file (배치·폴더 연결 API 라우터)
===========================================================
09_ETL_SFTP_Connection 설계서 기반. prefix /batch → /api/etl2/batch.
...
- POST /jobs/{id}/toggle — 활성/비활성 토글
- GET /jobs/{id}/history — 실행 이력 목록
- GET /jobs/{id}/history/{run_id} — 실행 이력 상세
- GET /jobs/{id}/skipped-files — 스킵/에러 파일 목록
- POST /jobs/{id}/skipped-files/delete — 원격 문제 파일 삭제 (Body: filenames)
