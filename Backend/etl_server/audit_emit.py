...
`sql_fingerprint` 는 생략 시 `audit_sql_catalog.etl_audit_sql_fingerprint` 로 채운다(`docs/main/04_DB_ARCHITECTURE.md` 13절 기본(1), 호출부 명시는 예외(2)).
...
=======================
- emit_etl_log(actor_user_id, *, business_action, action_kind=..., success_yn=..., detail_json=..., rows_affected=..., risk_tier=..., table_name=..., sql_fingerprint=...)

[Dependencies]
=========
- Backend.core.system_audit_log
- Backend.etl_server.audit_sql_catalog.etl_audit_sql_fingerprint
"""
...
from Backend.etl_server.audit_sql_catalog import etl_audit_sql_fingerprint
...
def emit_etl_log(
    ...
    *,
    ...
    fp = sql_fingerprint
    if fp is None:
        fp = etl_audit_sql_fingerprint(business_action, dj)
