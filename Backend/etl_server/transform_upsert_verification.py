...
[Main Functions]
===========
1. get_expected_pg_cast_for_series: Series dtype → PG 캐스트명
2. verify_transform_output_columns: DataFrame 컬럼·dtype 검증, 에러 문자열 목록 반환
3. run_dry_run_pipeline: apply_rules → apply_mapping_type_cast 드라이런, 검증 결과·dtype_map 반환

[Dependencies]
=========
- pandas
- Backend.etl_server.transform_engine (apply_rules, apply_mapping_type_cast)
"""
...
PANDAS_DTYPE_TO_PG_CAST: Dict[str, str] = {
    ...
    "datetime64[ns, Asia/Tokyo]": "timestamptz",
    "datetime64[ns, US/Eastern]": "timestamptz",
    "datetime64[ns, Europe/London]": "timestamptz",
    "object": "text",
    "string": "text",
    "category": "text",
...
if __name__ == "__main__":
    ...
    # Minimal dry run: small sample_df, one rule, one mapping (no Backend.etl_server package import)
