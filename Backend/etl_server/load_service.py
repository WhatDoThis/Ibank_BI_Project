...
[Main Functions]
===========
1. _resolve_upload_path: DB 경로가 현재 프로세스에서 없을 때 uploads/파일명으로 폴백 해석
2. _pg_type: inferred_type → PostgreSQL 타입 문자열
3. _read_file: file_path, file_type으로 CSV/Excel/Parquet 읽기. (DataFrame, data_verification_needed)
4. run_file_load: etl_table_id 기준 파일 읽기 → DROP+CREATE TABLE → 배치 INSERT → allowed_tables·job 갱신
5. run_file_upsert: 추가 적재. Job add_file_path 읽어 타겟 테이블에 PK 기준 ON CONFLICT DO UPDATE.
