...
[Main Functions]
===========
1. _run_one_job: Job 1건 실행. 파일/DB 분기 후 load_service 또는 db_load_service 호출. 예외 시에도 status failed 갱신(running 좀비 방지)
2. run_worker_iteration: running 수 < MAX_CONCURRENT인 만큼 claim 후 스레드 풀에 제출
3. _worker_loop: run_worker_iteration 주기 호출(POLL_INTERVAL_SEC), 예외 시 로그
4. start_background_worker: 백그라운드 스레드 + 스레드 풀 생성(앱 startup에서 1회)
5. stop_background_worker: 스레드 풀 shutdown(wait=True), 워커 루프 중단(앱 shutdown 훅에서 호출 권장)
