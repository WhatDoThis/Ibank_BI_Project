...
- **보기 모드**: 일반 / 일간·주간·월간·연간 비교. 기준·비교 일/주/월/연 선택(비어두면 전일/전 주/전 월/전년). **디멘션별 비교 (B) / 요약 보기 (A)** 토글. 기준별 발송·집계 테이블 복수 차원 시 **X축 단일 차원(일자 제외)**. KPI 순서 통일, 집계 테이블(컬럼 순서·rate 채우기 막대·내부 테두리·기준/비교 컬럼 배경). 위젯 생성: rate형 Y축 소수점 둘째자리·info 버튼. 상세는 **docs/main/00_PRD.md §6.2.1** 참고.
...
- **API**: FastAPI(health, report API, dashboard API, dashboard2 API), PostgreSQL 연동. 리포트: join-order, save-query-as-table·status 등.
...
- 웹: http://localhost:8080/ibank-bi/ (리포트: `/ibank-bi/report`, 대시보드: `/ibank-bi/dashboard`, 대시보드2: `/ibank-bi/dashboard2`, 위젯보드: `/ibank-bi/widgetboard`)
...
│   │   │       ├── dashboard/   # 대시보드1 (DashboardPage, KPI·도넛·막대·테이블·ChartWidget·ChartWidget2, periodCompare)
│   │   │       ├── dashboard2/  # 성과리포트 (주간/월간/일간/연간 비교, 디멘션별 비교/요약)
│   │   │       ├── widgetboard/# 위젯보드 (Dashboard3Page, 드래그 앤 드롭 그리드)
