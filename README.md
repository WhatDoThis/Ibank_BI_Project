...
- **필터**: 테이블 선택(필수 컬럼 만족 테이블만 노출), 기간·캠페인·워크플로우·채널(기본 "전체" 옵션), 집계 기준(일자/캠페인/워크플로우/채널).
- **주요 지표**: 캠페인 수·워크플로우 수·채널 수, 발송/성공/실패/오픈/클릭, 성공률·실패률·오픈률·클릭률(00.00% 포맷). 표시할 지표만 선택 가능(localStorage 저장).
