...
본 문서는 **docs/main** 내 **`user_dvsn`(조직 역할)·ETL 자격·프로젝트 권한 템플릿(`pmssn_*`)·가드(`require_permission` 등)** 전용 명세이며, **현재 코드 기준**으로만 정리한다.

- **작업 이력**: **docs/log/log.md**
- **용어 표준(`user_dvsn` vs `pmssn_*`)**: **docs/main/08_TERMINOLOGY.md**
- **DB 스키마·부서 트리**: **docs/main/04_DB_ARCHITECTURE.md**
- **알고리즘·API 순서(여정)**: **docs/main/06_CUSTOMER_JOURNEY.md**
- **엔드포인트·동기 처리**: **docs/main/03_API_GUIDE.md**
- **백엔드 패키지·설정**: **docs/main/02_BACKEND_GUIDE.md**
- **동작·계약의 기준**: **docs/main** 번호 문서(00~08). 권한·역할 규칙은 **본 문서(§0~§6)와 03·04·08**을 함께 본다. 저장소의 다른 위치에 개발 메모가 있더라도 불일치 시 **`docs/main`** 을 따른다.

**동시 저장·동기 처리**: 두 관리자가 같은 멤버 권한을 거의 동시에 바꿀 때 DB·알림이 어떻게 겹치는지는 **docs/main/03_API_GUIDE.md** §1.6 에만 적어 두었고, 본 문서에서는 반복하지 않는다.
...
pmssn_master_detail (권한 정의 1건씩, 시드)
        ↑
pmssn_master (프로젝트 권한 템플릿, pmssn_list TEXT[])
        ↑
...
| 저장값 | 한글 |
|--------|------|
| `sa_dev` | SA개발자 — 시드·전역 운영(화면 짧은 표기 등은 **08_TERMINOLOGY.md §1.1**) |
| `sa` | SA — 부서 슈퍼관리 |
| `a` | A — 부서 관리자 |
| `o` | O — 오퍼레이터(프로젝트 **생성·비활성·재활성·purge** 불가; 이름·설명·멤버 일부만) |
| `u` | U — 일반 사용자 |
...
- **배정 잠금**: 커스텀 `pmssn_master`가 `project_ptcpnt_info`에 한 명이라도 붙어 있으면 **`pmssn_list`(상세 권한) 변경은 API·UI에서 불가**이며, 권한명(`pmssn_name`)만 수정 가능하다.
...
**부록**: 시스템 DB **DDL 적용 순서** 등 운영 절차는 **docs/main/02_BACKEND_GUIDE.md** 및 사내 런북을 본다. **권한 판별 규칙은 본 가이드(§0~§6)가 기준**이다.
