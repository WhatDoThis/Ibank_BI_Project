...
- **GET `/api/system-logs`**: 쿼리 `user_key`(행위자 id 문자열·이메일 부분 일치), `from`·`to`(날짜), `ip_contains`, `page`(기본 1), `page_size`(기본 50, 최대 200), `channel`, `action_kind`, `success_yn`, 선택 `sort_by`(`create_dtm`|`system_log_id`|`channel`|`action_kind`|`success_yn`|`actor_user_id`), `sort_dir`(`asc`|`desc`, 기본 desc). **`from`·`to`를 모두 주면** (종료일−시작일) 일수가 **92일(약 3개월)** 을 넘기면 **400**. 응답 `{ items, total, page, page_size }`. **GET `/api/system-logs/export.csv`**: 동일 필터·정렬로 CSV, 건수 초과 시 **400**(상한 **50,000**행, `22` §8.1). 조회 범위: `sa_dev`는 전체, `sa`·`a`는 `actor_user_id`가 액터 부서 하위 트리(재귀 CTE)에 속한 활성 사용자인 행만(`actor_user_id` NULL 행은 제외).
