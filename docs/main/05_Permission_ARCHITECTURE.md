# 권한·조직 역할 아키텍처 (현행 코드 기준)

**용도**: `user_dvsn`(조직 역할)·ETL 자격·프로젝트 권한 템플릿을 **이 문서만**으로 파악 가능하게 정리한다.

**용어 표준**: `08_TERMINOLOGY.md`  
**참고 문서** (배경·스키마): `06_CUSTOMER_JOURNEY.md` · `04_DB_ARCHITECTURE.md`

**문서 정본**: 권한·역할 설명은 **`docs/main`** 이 정본이다. 저장소의 다른 위치에 있는 개발 메모와 불일치 시 **`docs/main`** 을 따른다.

**동시 저장·동기 처리**: 두 관리자가 같은 멤버 권한을 거의 동시에 바꿀 때 DB·알림이 어떻게 겹치는지는 `03_API_GUIDE.md` §1.6 에만 적어 두었고, 본 문서에서는 반복하지 않는다.
