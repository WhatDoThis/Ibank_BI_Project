...
- **백엔드 가이드**: 구조·API·etl_server 상세는 **docs/main/02_BACKEND_GUIDE.md** 참고. 프레임워크 전환 등 시간축 이력은 **docs/log/log.md**·Git을 본다.
...
| 24_Backend_admin_server_Module_Doc_And_Numbering_Audit.md | **백엔드 패키지 문서·`# N.` 전수 감사(단일 문서)**: **Part A** `admin_server`(13파일) 완료 기록·**Part B** 이후 `auth_server`·`project_server` 등은 **본 24번 파일에 섹션만 추가**하여 체크리스트·`compileall`·완료 표를 관리한다(별도 `25_` 파일 없음). |
